...
const styleSheet = createStyleSheet('ProductListPage', {
  actionsCell: {
    padding: 0,
    paddingRight: 2,
  },
...
const ProductListPage = ({ selectedIndex, classes, count, deleteProduct, list, open, openDeleteDialog, onRequestCloseDialog }) => (
  <div>
    <Table
      ...
      title="Products"
      toolbar={(
        <MuiIconButton aria-label="Add product" to="/products/create" component={Link}>
          <MuiIconAdd />
        </MuiIconButton>
      )}
    >
        ...
        cell={({ rowIndex }) => (
          <MuiTableCell classes={{ root: classes.actionsCell }} disablePadding>
            <div className={classes.actions}>
              <MuiIconButton aria-label={`Edit product ${list[rowIndex].name}`} to={`/products/${list[rowIndex]._id}`} component={Link}>
                <MuiModeEditIcon />
              </MuiIconButton>
              <MuiIconButton aria-label={`Detele product ${list[rowIndex].name}`} onClick={event => openDeleteDialog(rowIndex, event)}>
                <MuiDeleteIcon />
              </MuiIconButton>
            </div>
          </MuiTableCell>
    ...
    {selectedIndex !== undefined && (<MuiDialog open={open} onRequestClose={onRequestCloseDialog}>
      <MuiDialogTitle>Delete product?</MuiDialogTitle>
      <MuiDialogContent>
        <MuiTypography>
          You are about to delete product<b> {list[selectedIndex].name}</b>.
          <br />Are you sure?
        </MuiTypography>
      </MuiDialogContent>
      <MuiDialogActions>
        <MuiButton onClick={onRequestCloseDialog} color="default">Cancel</MuiButton>
        <MuiButton onClick={() => deleteProduct(list[selectedIndex]._id)} color="primary">Ok</MuiButton>
      </MuiDialogActions>
    </MuiDialog>)}
  </div>
)
...
export default withStyles(styleSheet)(ProductListPage)
